export const perguntas = [
    {
        enunciado: "Você gosta de filmes???",
        alternativas: [
            {
                texto: "Sim",
                afirmacao: [
                    "otimo! Você ficaria impressionado com o numero de pessoas que não gostam.",
                    "Muito bomm:)"
                    ],

                    proxima: 1,
            },

            {
                texto: "Não, eu prefiro desenho de criança",
                afirmacao: [
                    "Filmes as vezes podem parecer massantes, mas muitoas pessoas investem muito tempo e dedicação para sua construçãao, de a eles uma chance.",
                    "Ok, cada um tem seus gostos"
                    ],
                    proxima: 3,
            }           
            
        ],

    },
    {
        enunciado: "Você vê filmes com frequência?",
        alternativas: [
            {
                texto:"Sim,amo o mundo que entro vendo filmes! :)",
                afirmacao: [
                    "filme são Incriveis!",
                    "Você possui muita cultura"
                    ],

                    proxima: 2,

            },
            {
                texto: "Não, prefiro ver outra coisa",
                afirmacao: [
                    "Paia mano, mas ok",
                    "Ta ok!"
                    ],
                    proxima: 2,
            }
        ]
    },
    {
        enunciado: "Você acha que as pessoas por trás dos filmes merecem mais atenção?",
        alternativas: [
            {
                texto:"Sim",
                afirmacao: [
                    "Concordo totalmente com você!!",
                    "Você possui uma visão muito boa"
                    ],
                    proxima: 4,
            },
            {
                texto:"Não",
                afirmacao: [
                    "então blz brother",
                    "Vai trabalhar e sai daqui então"
                    ],
            }
            
        ]
    },
    {
        enunciado: "Você gosta de filmes de comédia",
        alternativas: [
            {
                texto: "Sim",
                afirmacao: [
                    "Que legal eu também",
                    "Te recomendo assistir o show de truman, é uma comédia com drama"
                    ],
                    proxima: 2,
            },
            {
                texto: "Não",
                afirmacao: [
                    "Tudo bem",
                    "Filha da mãe"
                    ],
                    proxima: 2,
            }           
            
        ]
    },
    {
        enunciado: "Você gosta do henry cavil ??",
        alternativas: [
            {
                texto: "Sim",
                afirmacao: [
                    "Bom gosto",
                    "Eu também!!"
                    ],
                    proxima: 5,
            },
            {
                texto: "Não",
                afirmacao: [
                    "Mano por que ?",
                    "Não venha mais aqui."
                    ]
            }           
            
        ]
    },
    {
        enunciado: "Gostaria de participar de um mini quiz sobre ele??",
        alternativas: [
            {
                texto: "Sim, vamo lá vou",
                afirmacao: [
                    "Vamos ver se você sabe então",
                    "VAMOSSSs"
                    ],

                    proxima: 6,
            },
            {
                texto: "Não to afim",
                afirmacao: [
                    "tudo bem",
                    "tudo bem então"
                    ]
            }           
            
        ]
    },

    {
        enunciado: "Qual esporte ele praticava",
        alternativas: [
            {
                texto: "rugby",
                afirmacao: [
                    "Boa!",
                    "Acertou!"
                    ],
                
                proxima: 7,
            },
            {
                texto: "ping bong",
                afirmacao: [
                    "O cara não fuma não mano",
                    "Você está errado, a proxima vai!"
                    ],
                proxima: 7,
            }           
            
        ]
    }, 
    
    {
        enunciado: "Por que ele sofria bullingy na infancia?",
        alternativas: [
            {
                texto: "Por ser muito inteligente",
                afirmacao: [
                    "errou feio errou rude",
                    "ele era, mas não era isso"
                    ],
                
                proxima: 8,
            },
            {
                texto: "Por estar acima do peso",
                afirmacao: [
                    "ACERTOU, as pessoas não deveriam fazer isso",
                    "Mandou bem!"
                    ],
                proxima: 8,
            }           
            
        ]
    },
    

{

    enunciado: "Qual destes filmes ele não fez",
        alternativas: [
            {
                texto: "Deadpool e Wolwerine",
                afirmacao: [
                    "Ele fez este, foi mal pelo spoiller",
                    "Uma pequena aparação, perdão pelo spoiller",
                    ],
                proxima: 9,
            },
            {
                texto: "Vovozona 4",
                afirmacao: [
                    "Nem tem vovozona 4.... eu saberia",
                    "Seria legal mas não",
                    ],
                proxima: 9,
            }          
            
        ]
    },
    {
    enunciado: "Qual é o jogo favorito dele?",
        alternativas: [
            {
                texto: "Warhammer 40000",
                afirmacao: [
                    "Acertou, essa era alto nivel",
                    "VAAAPOOO acertou",
                    ],
                proxima: 10,
            },
            {
                texto: "Street fighter",
                afirmacao: [
                    "Pior que não é",
                    "errou feio errou rude",
                    ],
                proxima: 10,
            }          
            
        ]
    },

    {
        enunciado: "Gostou do quiz?",
            alternativas: [
                {
                    texto: "Sim",
                    afirmacao: [
                        "VAPOOO ai sim meu fi!!!",
                        "Obrigado pela participação!!!",
                        ]
                },
                {
                    texto: "Não",
                    afirmacao: [
                        "Se prepare para correr então",
                        "Vou até a sua casa mudar essa opnião",
                        ]
                }          
                
            ]
        },

];
